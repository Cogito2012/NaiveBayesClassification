function [train_data,train_label,test_data,test_label] = prepare_cls_data(data,data_gt,feat_bands,sample_ratio,rm_threshold)
% remove samples with less than a given threshold, and randomly select a
% fixed ratio of samples for training set and the rest of all for test set
% for each class(Land cover type).
if nargin < 3, feat_bands = 1:size(data, 3); end
if nargin < 4, sample_ratio = 0.5; end
if nargin < 5, rm_threshold = 100; end
...
for i=new_clsidx_list
    ...
    train_num_inclass = round(length(r)*sample_ratio);
    % get training samples index
    r_train = r(1:train_num_inclass);
    c_train = c(1:train_num_inclass);
    % get test samples index
    r_test = r(train_num_inclass+1:end);
    c_test = c(train_num_inclass+1:end);
